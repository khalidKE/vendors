...
class RegisterViewModel extends MyBaseViewModel with QrcodeScannerTrait {
  ...
  Future<List<Address>> searchAddress(String keyword) async {
    print("search addres called: $keyword");
  ...
  fetchVendorTypes() async {
    ...
    try {
      ...
      vendorTypes =
          vendorTypes
              .where((e) => !e.slug.toLowerCase().contains("taxi"))
              .toList();
    } catch (error) {
      toastError("$error");
    }
  ...
  pickBusinessLogo() async {
    ...
    if (selectedFile != null) {
      logo = File(selectedFile.path);
      notifyListeners();
    }
  }

  pickBusinessFeatureImage() async {
    final selectedFile = await imagePicker.pickImage(
      source: ImageSource.gallery,
    );
    if (selectedFile != null) {
      featureImage = File(selectedFile.path);
      notifyListeners();
    }
  }
